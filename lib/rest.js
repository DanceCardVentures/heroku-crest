...
var MongoClient = require("mongodb").MongoClient,
  ...
  https_server = module.parent.exports.https_server,
...
function handleGet(req, res, next) {
  debug("GET-request received " + req.getPath())// + JSON.stringify(req));
  var query;
...
https_server.get('/:db/:collection/:id?', handleGet);
https_server.get('/:db/:collection', handleGet);
 ...
 * Insert
 */
https_server.post('/:db/:collection', function (req, res) {
  debug("POST-request recieved");
  if (req.params) {
    MongoClient.connect(util.connectionURL(req.params.db, util.getAuthenticationFromRequest(req), config), function (err, db) {
      if(err){
 ...
 * Update
 */
https_server.put('/:db/:collection/:id', function (req, res) {
  debug("PUT-request recieved");
  var spec = {
    '_id': mongo.ObjectID.createFromHexString(req.params.id)
  };
 ...
 * Delete
 */
https_server.del('/:db/:collection/:id', function (req, res) {
  debug("DELETE-request recieved");
  var spec = {
    '_id': mongo.ObjectID.createFromHexString(req.params.id)
  };
